...
class TranscriberController(QObject):
    ...
    def _load_parakeet_params(self) -> None:
        self.transcription_service.set_parakeet_params(
    ...
    def set_parakeet_params(self, segment_length: int, segment_duration: int,
    ...
    def _connect_signals(self) -> None:
        ...
        self.model_manager.server_model_loaded.connect(self._on_server_model_loaded)
    ...
    def update_model(self, model_name: str, precision: str, device: str) -> None:
        self.enable_widgets_signal.emit(False)
        self.model_manager.load_model(model_name, precision, device)
    ...
    def start_batch_processing(self, files, output_format, output_directory,
                                segment_length, segment_duration, include_timestamps) -> None:
        ...
        current_model_name = config_manager.get_value("model_name", "Parakeet TDT 0.6B v2")
        model_type = ModelMetadata.get_model_type(current_model_name)
        effective_segment_length = ModelMetadata.get_chunk_length(current_model_name) \
            if model_type == "canary" else segment_length
    ...
    @Slot(str, str, str)
    def _on_server_model_loaded(self, name: str, precision: str, device: str) -> None:
        self.transcription_service.set_model_type(ModelMetadata.get_model_type(name))
    ...
    def _on_transcription_progress(self, segment_num: int, total_segments: int, percent: float) -> None:
        if percent >= 0:
            ...
            self.update_button_signal.emit(f"Transcribing... chunk {segment_num}")
