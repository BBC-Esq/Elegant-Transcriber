...
from PySide6.QtCore import Qt, Slot, QRect, QEvent, Signal, QSettings, QByteArray, QTimer, QSize
...
def _detect_supported_precisions(cuda_available: bool) -> dict[str, list[str]]:
    result = {"cpu": ["float32"], "cuda": []}
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                pass
            result["cuda"] = precisions
    except Exception as e:
        logger.warning(f"Failed to detect CUDA precisions: {e}")
    return result
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, cuda_available: bool = False):
        ...
        self.setWindowTitle("Elegant Recorder")

        self.settings = QSettings("ElegantRecorder", "Recorder")
        ...
        self.supported_precisions: dict[str, list[str]] = {"cpu": [], "cuda": []}
    ...
    def _validate_precision(self, precision: str, model_name: str, device: str) -> str:
        available = ModelMetadata.get_precision_options(model_name, device, self.supported_precisions)
        if precision in available:
            return precision
        if available:
            logger.warning(
                f"Precision '{precision}' not available for {model_name}/{device}, using {available[0]}"
            )
            return available[0]
        logger.warning(f"No precisions available for {model_name}/{device}, using float32")
        return "float32"
    ...
    def _restore_state(self) -> None:
        ...
        self._clipboard_visible = self.settings.value(
            SETTINGS_CLIPBOARD_VISIBLE, self.DEFAULTS["clipboard_visible"], type=bool
        )
        ...
        self._file_panel_visible = self.settings.value(
            SETTINGS_FILE_PANEL_VISIBLE, self.DEFAULTS["file_panel_visible"], type=bool
        )
    ...
    def _save_state(self) -> None:
        ...
        self.settings.setValue(SETTINGS_FILE_PANEL_DOCKED, self.file_panel.is_docked())
    ...
    def _show_current_model_status(self) -> None:
        if self._model_is_loaded:
            ...
            precision = self.loaded_model_settings.get("precision", "")
            device = self.loaded_model_settings.get("device_type", "")
            self._update_model_status(f"{name} ({precision} / {device})")
        else:
            self._update_model_status("No model loaded")
    ...
    def _on_model_download_started(self, model_name: str, total_bytes: int) -> None:
        ...
        total_str = self._format_bytes(total_bytes) if total_bytes else "unknown size"
    ...
    def _on_parakeet_settings_changed(self, settings: dict) -> None:
        for key, value in settings.items():
            self._save_config(key, value)
        self.controller.set_parakeet_params(
            segment_length=config_manager.get_value("segment_length", 90),
            segment_duration=settings.get("segment_duration", 10),
            include_timestamps=settings.get("include_timestamps", False),
        )
    ...
    @Slot(str, str, str)
    def _on_model_loaded_success(self, model_name: str, precision: str, device: str) -> None:
        self._is_loading_model = False
    ...
    def _on_result_ready(self, result) -> None:
        ...
        try:
            ...
            QMessageBox.warning(self, "Output Error", f"Failed to write output file:\n{e}")
    ...
    def _sync_side_windows(self) -> None:
        ...
        if self.file_panel.isVisible() and self.file_panel.is_docked():
    ...
    def closeEvent(self, event):
        ...
        if self.controller.is_transcribing() or self.controller.is_batch_processing():
            reply = QMessageBox.question(
                ...
                "Click OK to cancel it and exit, or Cancel to keep the app open.",
