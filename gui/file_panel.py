...
from PySide6.QtGui import QMoveEvent, QPainter, QColor, QStandardItemModel
from PySide6.QtCore import QSize as _QSize


_CANARY_NO_TIMESTAMPS_TIP = (
    "<qt>Canary does not support timestamps.<br>"
    "SRT/VTT require timestamps, so they<br>"
    "are disabled while Canary is loaded.</qt>"
)
_TIMESTAMP_FORMATS = {"srt", "vtt"}
...
class FilePanelWindow(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None, width: int = 280):
        ...
        self._timestamps_supported: bool = True
        ...
        grid.addWidget(self._format_combo, 0, 1, 1, 3)
        ...
        for i in range(self._format_combo.count()):
            ...
            if fmt in _TIMESTAMP_FORMATS:
                if supported:
                    ...
                    item.setFlags(item.flags() & ~Qt.ItemIsEnabled)
                    item.setToolTip(_CANARY_NO_TIMESTAMPS_TIP)

        if not supported and self._format_combo.currentText() in _TIMESTAMP_FORMATS:
            txt_idx = self._format_combo.findText("txt")
            if txt_idx >= 0:
                self._format_combo.setCurrentIndex(txt_idx)
    ...
    def _on_start(self) -> None:
        ...
        if self._is_single_mode():
            ...
            self.transcribe_file_requested.emit(
                self._selected_path, mode, fmt, output_dir
            )
        else:
            ...
            self.batch_start_requested.emit(files, fmt, output_dir)
    ...
    def on_batch_completed(self, message: str) -> None:
        ...
        if self._batch_error_count > 0:
            summary = (
                f"Completed with {self._batch_error_count} error"
                f"{'s' if self._batch_error_count != 1 else ''} — {message}"
            )
    ...
    def on_batch_error(self, message: str) -> None:
        ...
        self._last_batch_error = message
